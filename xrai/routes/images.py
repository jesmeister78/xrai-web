...
from xrai.constants import ATTR_MAP
...
def upload_image():
    ...
    if request.method == 'POST':
        ...
        if file and allowed_file(file.filename):
            ...
            return render_template('show_image.html', path=filename)
...
def show_processed_images():
    images = os.listdir(os.path.join(current_app.root_path, current_app.config['STATIC_FOLDER'], current_app.config['TEMP_OUTPUT_FOLDER']))
    images = ['images/processed/' + file for file in images]
    return render_template('show_processed_images.html', images = images)
...
def show_sample_images():
    ...
    return render_template('show_samples.html', images = images)
